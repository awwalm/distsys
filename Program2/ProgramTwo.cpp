...
* @title        : Program Two - Back and Forth / two-Way Data Transfer
* @date         : 28-11-19
*/

/*! \brief Program 2 (ProgramTwo.cpp)
 *         Copies bytes of data from one buffer to another buffer,
 *         which then copies the bytes back to the original source,
 *         all within the same shared memory space.
 *
 * This two-way buffer to buffer data transfer program performs a benchmarking test.
 * The results are intitially displayed in the console application or command line.
 * Python code is generated for the graphing and executed as C++ finishes the benchmarking.
 * 
 */
 ...
 *    IMAX actually has TWO but not one functions:
 *        1. Specifies that the integers be copied 1000 times (although, the cascading condition terminates before this can be done).
 *        2. Utilize the assigned value to scale up extremely small values like latency and bandwidth by multiplicaion.
 *
 */
...
int main(int argc,char **argv) /**< Begin Main Function. */
{
  ...
  */
  ...
  {
        std::cout << "ALL GRAPHING FILES (BytesGraph.py, IterationsGraph.py, TimeGraph.py, LatencyGraph.py, BandwidthGraph.py, CombinedGraph.py, BandwidthGraph.py, CombinedGraph.py) DISCOVERED." << endl;
  ...
  std::cout << "GENERATING PYTHON FILE NAMES (BytesGraph.py, IterationsGraph.py, TimeGraph.py, LatencyGraph.py, BandwidthGraph.py, CombinedGraph.py)... \n";
  ...
  //---------------------------------------------------------
    ...
    for (bytes = 0; bytes <= bmax; bytes = (bytes) ? bytes << 1 : 1) /**< Start critical For-loop. */
     {
        ...
        for (iterations = 0; iterations < imax; iterations++) /**< Start secondary For-loop */
        {
            ...
            *        Then, the bytes are copied back yet again from @param outputBuffer back to @param inputBuffer.
            *
            * This function returns an array, in this case, explcitly casted as int* rather than char*, of characters.
            */
            memcpy(outputBuffer,inputBuffer,bytes);
            memcpy(inputBuffer, outputBuffer, bytes);
        }

        time = log_ClockTime() - time;
    ...
    txtOut << "plt.title('Byte Readings for Program 2')\n";
    txtOut << "plt.legend()\n";
    txtOut << "plt.show()\n";
    ...
    txtOut2 << "plt.title('Iterations Readings for Program 2')\n";
    txtOut2 << "plt.legend()\n";
    txtOut2 << "plt.show()\n";
   ...
   // ShellExecute(NULL, "open", "IterationsGraph.py", NULL, NULL, SW_SHOWDEFAULT); /**< No need to immediately execute the Python file right away. */
    ...
    txtOut3 << "plt.title('Time Readings for Program 2')\n";
    txtOut3 << "plt.legend()\n";
    txtOut3 << "plt.show()\n";
    ...
    txtOut4 << "plt.title('Latency Readings for Program 2')\n";
    txtOut4 << "plt.legend()\n";
    txtOut4 << "plt.show()\n";
 ...
 //-------------------------------------------------------------------------------------------------------
    ...
    txtOut5 << "plt.title('Bandwidth Readings for Program 2')\n";
    txtOut5 << "plt.legend()\n";
    txtOut5 << "plt.show()\n";
 ...
 //-------------------------------------------------------------------------------------------------------
    ...
    txtOutC << "plt.title('Combined Comparison Chart For Program 2')\n";
    txtOutC << "plt.legend()\n";
    txtOutC << "plt.show()\n";
   ...
   //-------------------------------------------------------------------------------------------------------
    ...
    txtOut7 << "plt.title('Bytes vs Bandwidth for Program 2')\n";
    txtOut7 << "plt.legend()\n";
    txtOut7 << "plt.show()\n";
   ...
   //-------------------------------------------------------------------------------------------------------
    ...
    txtOut8 << "plt.title('Bytes vs Latency for Program 2')\n";
    txtOut8 << "plt.legend()\n";
    txtOut8 << "plt.show()\n";
